...
pj_status_t pj_zmodo_stun_msg_encode(pj_stun_msg *msg,
				       pj_uint8_t *buf, pj_size_t buf_size,
				       unsigned options,
				       const pj_str_t *key,
				       pj_size_t *p_msg_len);

void encrypt_zmodo_stun_msg(char* src, int* len, char** dest);

void decrypt_zmodo_stun_msg(char* src, int* len, char** dest);
/**
...
typedef struct zmodo_stun_msg_hdr
{
    ...
    unsigned int length;
}zmodo_stun_msg_hdr;//network byte order
/**
 * @}
 */
